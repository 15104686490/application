...
public class NewVersionDocServiceImpl implements DocService {
   ...
   /* @Autowired
        ...
        try {
            ...
            for (XWPFParagraph xp : paragraph) {
                ...
                for (String str : textList) {
                    LinkedHashMap<String, String> paragraphResult = new LinkedHashMap<>();
                    boolean f = false;
                    /*完整规范名称*/
                            ...
                            if (!rulesService.queryFullNameList().contains(onlyStringTemp)) {
                                ...
                                if (!codeFlag) {
                                    for (String fullCode : rulesService.queryFullCodesSet()) {
                                        ...
                                        String startOfOnlyString = getStartOfCode(onlyString);

                                        if (adviceCode.equals("") && (!startOfFullCode.equals("*") && (!startOfOnlyString.equals("*")))
                                                && startOfOnlyString.endsWith(startOfFullCode)) {
                ...
                for (XWPFTableRow xwpfTableRow : xwpfTableRowList) {
                    ...
                    for (XWPFTableCell xwpfTableCell : xwpfTableCells) {
                        ...
                        for (XWPFParagraph xp : cellParagraphs) {
                            ...
                            for (String str : textList) {
                                boolean f = false;
                                TreeMap<String, String> paragraphResult = new TreeMap<>();
                                for (Pattern p : langConfigsPattern) {
                                    Matcher matcher = p.matcher(str);
                                    /*log.error();*/
                                    while (matcher.find()) {
    ...
    public String getMd5OfString(String txtValue) {
        if (txtValue != null) {
            String res = DigestUtils.md5Hex(txtValue);
            return res;
        } else {
            return "*";
        }
